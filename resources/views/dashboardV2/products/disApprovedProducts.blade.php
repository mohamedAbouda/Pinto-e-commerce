...
<div class="row">
    <div class="col-md-4 col-xs-12">
        <h3 class="section-title contacts-section-title">{{ trans('web.dashboard_products_disapproved_dis_products') }} </h3>
    </div>
    ...
    <div class="col-md-4 col-md-offset-1 text-right col-xs-11">
        ...
        <a class="btn btn-blue margin-left-10" href="{{route('dashboard.products.create')}}">
            <span>+ </span>{{ trans('web.dashboard_products_disapproved_add') }}
        </a>
        @endif
        ...
        <div class="btn-group">
            ...
            <ul class="dropdown-menu contact-dropdown pull-right" role="menu">
                    ...
                    <li>
                    <a href="{{route('dashboard.products.featured.products')}}">{{ trans('web.dashboard_products_disapproved_feat') }}</a>
                </li>
                <li>
                    <a href="{{route('dashboard.show.approved.products')}}">{{ trans('web.dashboard_products_disapproved_app') }}</a>
                </li>
                <li>
                    <a href="{{route('dashboard.show.disapproved.products')}}">{{ trans('web.dashboard_products_disapproved_dis') }}</a>
                </li>
            </ul>
        </div>
        @endif
            ...
            <div class="col-md-4 margin-bottom10 margin-top20">
                <div class="total-customer-col pad5 pad-bottom5 col-md-12">
                    <div class="col-md-9 customer-stat-col-pad">
                        <h5 class="customer-stat-text pad5">{{ trans('web.dashboard_products_disapproved_tot') }}</h5>
                    </div>
            ...
            <table class="table table-borderless table-responsive">
                <tbody>
                    <tr>
                        ...
                        <th>
                            {{ trans('web.dashboard_products_disapproved_name') }}
                        </th>
                        
                        <th>
                            {{ trans('web.dashboard_products_disapproved_action') }}
                        </th>
                    </tr>
                  ...
                  <a href="{{url('dashboard/products/'.$product->id.'/destroy')}}"
                    ...
                    <a href="{{url('dashboard/products/'.$product->id.'/reviews')}}"
                     class="btn btn-sm btn-success" data-toggle="tooltip" data-placement="top"
                     title="Reviews"><i class="fa fa-eye"></i> {{ trans('web.dashboard_products_disapproved_rev') }}</a>
                      @if(!Auth::guard('merchant')->check())
                      @if($product->approved == 1)
                         {{ Form::open(['route' => 'dashboard.admin.toggle.approve']) }}
                         <input type="hidden" name="product_id" value="{{$product->id}}">
                                        <button type="submit" class="btn btn-danger">{{ trans('web.dashboard_products_disapproved_disapp') }}</button>
                                        {{ Form::close() }}
                      @else
                      {{ Form::open(['route' => 'dashboard.admin.toggle.approve']) }}
                         <input type="hidden" name="product_id" value="{{$product->id}}">
                                        <button type="submit" class="btn btn-primary">{{ trans('web.dashboard_products_disapproved_approve') }}</button>
                                        {{ Form::close() }}
                      @endif
                      @endif
