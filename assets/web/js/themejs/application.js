	...
	-------------------------------------------------- */
	    ...
	    $(".collapsed-block .expander").on('click', function (e) {
			...
			if (!$(collapse_content_selector).hasClass("open")) {
				$(collapse_content_selector).addClass('open').show('slow');
				expander.addClass("open").html("<i class='fa fa-angle-up'></i>") ;
			}
			else {
				$(collapse_content_selector).removeClass('open').hide('slow');
				expander.removeClass("open").html("<i class='fa fa-angle-down'></i>");
			}
	    	// setTimeout (function(){
			// },2000);
	    })
...
$(document).ready(function($) {
	...
	-------------------------------------------------- */
		$('.slideshow').owlCarousel2({
			loop:$('.slideshow').find('.items').length > 1?true:false,
			margin:0,
...
$(document).ready(function(){
	...
	// var austDay = new Date('2018-2-28');
	// console.log(austDay);return;
	$('.defaultCountdown-30').each(function(index){
		var date = $(this).attr('data-activation-end');
		if (!date) {
			return;
		}
