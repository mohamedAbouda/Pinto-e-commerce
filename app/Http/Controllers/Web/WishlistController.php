...
class WishlistController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        if(Auth::guard('client')->check()){
            $check = WishList::where('client_id',Auth::guard('client')->user()->id)->where('product_id',$request->input('id'))->first();
            if(!$check){
            ...
            return ['Product Added to wish list.',$count];
            }else{
                $count = WishList::where('client_id',Auth::guard('client')->id())->count();
                return ['this product already in wish list.',$count];
            }
        }else{
            return ['please login to add products to wish list.',0];
        }
    }
}
